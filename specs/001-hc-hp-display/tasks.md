---
description: "Task list for MMM-HC_HP — Affichage HC/HP depuis myelectricaldata"
---
...
- [x] T001 Créer la structure de fichiers du module selon plan.md : `MMM-HC_HP.js`, `node_helper.js`, `MMM-HC_HP.css`, `package.json`, `.eslintrc.js`, `.gitignore`, `tests/fixtures/`, `cache/` (gitignored)
- [x] T002 [P] Rédiger `package.json` avec les métadonnées MagicMirror² (`name`, `version`, `description`, `main: "MMM-HC_HP.js"`, `author`, `license`) et la dépendance `node-fetch@^2`
- [x] T003 [P] Créer `.eslintrc.js` avec les règles ESLint pour Node.js CommonJS (`node_helper.js`) et ES6 navigateur (`MMM-HC_HP.js`) — interdire `console.log` au profit de `Log.*`
...
- [x] T006 [P] Créer le squelette de `MMM-HC_HP.css` avec les sélecteurs vides : `.MMM-HC_HP`, `.mmm-hc-hp-badge`, `.mmm-hc-hp-badge--hc`, `.mmm-hc-hp-badge--hp`, `.mmm-hc-hp-list`, `.mmm-hc-hp-list__item`, `.mmm-hc-hp-cache-notice`, `.mmm-hc-hp-error`
...
- [x] T008 Scaffolder `MMM-HC_HP.js` avec `Module.register("MMM-HC_HP", { defaults: { updateInterval: 86400000, timeFormat: 24, animationSpeed: 1000 }, start() {}, getDom() { return document.createElement("div"); }, socketNotificationReceived(notification, payload) {} })` ; initialiser l'état interne `this._state = { periods: [], fetchedAt: null, fromCache: false, error: null, currentType: null }`
...
- [x] T010 Implémenter le chemin d'erreur IPC dans `node_helper.js` (`this.sendSocketNotification("HCHP_ERROR", { message, code })`) et la réception dans `MMM-HC_HP.js` (`socketNotificationReceived` → stocker `this._state.error`, appeler `this.updateDom()`)

**Checkpoint** : `node_helper.js` et `MMM-HC_HP.js` chargent sans erreur dans MagicMirror (le module affiche un div vide, les logs montrent le stub IPC).
...
- [x] T012 [P] [US1] Implémenter `getCurrentType(periods, now)` dans `MMM-HC_HP.js` — convertir `now` en minutes depuis minuit, itérer les plages HC, gérer le chevauchement minuit (`end < start`), retourner `"HC"` ou `"HP"` (research.md R-005)
...
- [x] T014 [US1] Câbler `MMM-HC_HP.js` : dans `socketNotificationReceived("HCHP_CONTRACT_DATA")`, stocker `periods`, `fetchedAt`, `fromCache` dans `this._state`, calculer `currentType` via `getCurrentType()`, appeler `this.updateDom(this.config.animationSpeed)`
- [x] T015 [US1] Implémenter `getDom()` dans `MMM-HC_HP.js` — section badge : élément `.mmm-hc-hp-badge` avec texte « ⚡ Heures Creuses » ou « ⚡ Heures Pleines », classes CSS `.mmm-hc-hp-badge--hc` / `--hp` selon `this._state.currentType` ; afficher « Chargement… » si `periods` vide et pas d'erreur
- [x] T016 [US1] Ajouter `setInterval(() => { this._state.currentType = getCurrentType(this._state.periods, new Date()); this.updateDom(0); }, 60000)` dans `start()` de `MMM-HC_HP.js` ; envoyer `HCHP_FETCH_CONTRACT` au démarrage
- [x] T017 [US1] Styliser le badge HP/HC dans `MMM-HC_HP.css` : `.mmm-hc-hp-badge--hc` en bleu (`#1565c0`, police bold) ; `.mmm-hc-hp-badge--hp` en orange (`#e65100`, police bold) ; taille de police ≥ 1.4em ; padding genereux pour lisibilité à distance
...
- [x] T019 [US2] Étendre `getDom()` dans `MMM-HC_HP.js` — section liste `.mmm-hc-hp-list` : itérer `this._state.periods`, créer un `.mmm-hc-hp-list__item` par plage avec le label `type + " " + label`, appliquer classe `--hc` ou `--hp` selon le type ; la liste est rendue **sous** le badge
- [x] T020 [US2] Styliser la liste dans `MMM-HC_HP.css` : `.mmm-hc-hp-list__item--hc` fond bleu clair / texte sombre ; `.mmm-hc-hp-list__item--hp` fond orange clair / texte sombre ; espacement vertical entre items ; largeur max 100% du module
...
- [x] T024 [US3] Implémenter `getDom()` dans `MMM-HC_HP.js` — section cache notice : si `this._state.fromCache === true`, afficher `.mmm-hc-hp-cache-notice` avec « Données du : [date formatée de fetchedAt] » sous la liste
- [x] T025 [US3] Implémenter `getDom()` dans `MMM-HC_HP.js` — section erreur : si `this._state.error !== null` et `periods` vide, afficher `.mmm-hc-hp-error` avec le message approprié (`"⚠ Token manquant — vérifiez votre configuration"` pour 401, `"⚠ PRM incorrect"` pour 404, `"⚠ Aucune donnée disponible"` générique) — FR-009
- [x] T026 [US3] Styliser les nouveaux éléments dans `MMM-HC_HP.css` : `.mmm-hc-hp-cache-notice` en gris clair, police ≤ 0.8em, italique ; `.mmm-hc-hp-error` en rouge clair, police 1em, icône ⚠ visible
...
- [x] T029 Passer ESLint sur `MMM-HC_HP.js` et `node_helper.js` (`npx eslint MMM-HC_HP.js node_helper.js`) et corriger toutes les erreurs jusqu'à zéro warning
